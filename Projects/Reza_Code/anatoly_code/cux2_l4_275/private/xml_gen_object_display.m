function xml_gen_object_display( obj_struct,indent )
%
% xml_gen_object_display - general function to display an object's content
%
% format:   xml_gen_object_display( obj_struct,indent )
%
...
%
% example:  xml_gen_object_display( struct( my_object_handle) );
%           xml_gen_object_display( ny_structure );
%
% Correction History:
...
%     xml_gen_object_display( obj_struct{i},indent + 2 );
%   end
%   return
% end
if ~isstruct( obj_struct )
  ...
  for idx = 1:length( field_list )
    ...
    case iscell( temp ),    
      if (length(temp)<10)
        ...
        %disp(temp)
        for i =1:length(temp)
          xml_gen_object_display( temp{i},indent + max_strlen + 2 );
        end
      else
        fprintf( '%s[%dx%d cell]\n',name,size(temp,1),size(temp,2) );
      end
    case isstruct( temp ),  fprintf( '%s[%dx%d struct]\n',name,size(temp,1),size(temp,2) );
        if (indent<80)
            xml_gen_object_display( temp,indent + max_strlen + 2 );
        end
