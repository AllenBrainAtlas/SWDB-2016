...
%                              'a6', 'a7', 'a8', 'a9', 'a10'
%                     
%                     paper size can also be any custo size defined as
%                     widthxheight with values in inches, for example
%                     '10x12.5'
%                         
...
% 
%      May 26, 2013: Thomas Deneux, improved paper size handling
...
function gsData = LocalParseArgs(varargin) 
    ...
    for i = 1 : 2 : length(varargin)-1 
        ...
        switch(lower(param_arg)) 
            ...
            % paper size 
            case 'gspapersize'
                paperSizes = {'letter', 'ledger', 'legal', '11x17', 'archA', 'archB', ...
                    'archC', 'archD', 'archE', 'a0', 'a1', 'a2', 'a3','a4', 'a5', ...
                    'a6', 'a7', 'a8', 'a9', 'a10'};
                ok = ismember(param_value,paperSizes) || ~isempty(regexp(param_value,'^\d+(\.\d+){0,1}x\d+(\.\d+){0,1}$', 'once'));
                if ~ok
                    warning('ps2pdf:papersize', ...
                        '''gspapersize'' value <%s> not found in the list of known sizes, neither of the form withxheight, ignoring it.', param_value);
                else
                    gsData.paperSize = param_value;
                end
    ...
    if ~isfield(gsData, 'cmd') 
        ...
        if ~exist(ghostDir,'dir') % Thomas Deneux
            ghostDir = 'C:\Program Files (x86)\MATLAB\R2013a Student\sys\gs8x';
        end
...
function gsData = LocalCreateResponseFile(gsData) 
   ...
   if isfield(gsData, 'paperSize') 
       paperSizes = {'letter', 'ledger', 'legal', '11x17', 'archA', 'archB', ...
           'archC', 'archD', 'archE', 'a0', 'a1', 'a2', 'a3','a4', 'a5', ...
           'a6', 'a7', 'a8', 'a9', 'a10'};
       if ismember(gsData.paperSize,paperSizes) 
           fprintf( rsp_fid, '-sPAPERSIZE=%s\n', gsData.paperSize );
       else
           tokens = regexp(gsData.paperSize,'^(\d+(\.\d+){0,1})x(\d+(\.\d+){0,1})$', 'tokens');
           winches = str2double(tokens{1}{1});
           hinches = str2double(tokens{1}{2});
           fprintf( rsp_fid, '-dDEVICEWIDTHPOINTS=%i\n-dDEVICEHEIGHTPOINTS=%i\n', round(winches*72), round(hinches*72));
       end
   end
